...
select  em.employee_id "직원번호",
        em.first_name "이름",
        em.salary "급여",
        round(salaryInfo.avgSalary,0) "평균급여",
        salaryInfo.maxSalary "최대급여"
from    employees em,(select avg(salary) avgSalary,
                             max(salary) maxSalary
                      from    employees
) salaryInfo
where   em.salary >= salaryinfo.avgsalary
...
select  jb.job_title,
        jobSal.sumSal
from    jobs jb, (select    sum(salary) sumSal,
                            job_id
                  from    employees
                  group by job_id) jobSal
where   jb.job_id = jobSal.job_id
order by jobSal.sumSal desc;
...
select  em.employee_id "사번",
        em.first_name "이름",
        em.salary "급여",
        round(de.avgsal,0) "부서평균급여" --화인용
from    employees em, (select   avg(salary) avgSal,
                                department_id
                       from    employees
                       group by department_id) de
where   em.department_id = de.department_id and em.salary>de.avgSal;
