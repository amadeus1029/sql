...
select  em.employee_id "사번",
        ...
        de.department_name "부서명",
        jb.job_title "현재업무"
from    employees em, jobs jb, departments de
...
select  rg.region_name "지역이름",
        co.country_name "나라이름"
from    regions rg, countries co
where   rg.region_id = co.region_id
...
select  em.employee_id "사번",
        ...
        jh.job_id "업무아이디", --현재업무 뽑히지 않도록 주의할것
        jh.start_date "시작일",
        jh.end_date "종료일"
...
select  de.department_id "부서번호",
        ...
        co.country_name "나라",
        ro.region_name "지역"
from    departments de, employees em, locations lo, countries co, regions ro
where   em.employee_id = de.manager_id and de.location_id = lo.location_id and lo.country_id = co.country_id and co.region_id = ro.region_id;
